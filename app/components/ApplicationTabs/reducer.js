import { NavigationExperimental } from 'react-native';
import { JUMP_TO_TAB } from '../../lib/navigation/constants';

const { StateUtils } = NavigationExperimental;

const simpleIcon = {
	uri: 'data:image/png;base64,',
	scale: 3
};

const tabs = {
  routes: [
    { key: 'feed', icon: simpleIcon, title: 'Items' },
    { key: 'notifications', icon: simpleIcon, title: 'Notifications' },
    { key: 'settings', icon: simpleIcon, title: 'Settings' }
  ],
  key: 'ApplicationTabs',
  index: 0
};

module.exports = (state = tabs, action) => {
  if (action.payload && action.payload.key !== tabs.key) {
    return state;
  }

  switch (action.type) {
    case JUMP_TO_TAB:
      return StateUtils.jumpToIndex(state, action.payload.tabIndex);
    default:
      return state;
  }
};
